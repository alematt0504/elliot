...
class RP3beta(RecMixin, BaseRecommenderModel):
    ...
    def get_user_predictions(self, user_id, mask, top_k=10):
        ...
        # ------ LOCAL K ------ #
        valid_items = int(np.sum(mask[user_id]))  # Count of valid (non-masked) items
        local_k = min(top_k, valid_items)  # Adjust k if fewer valid items are available
