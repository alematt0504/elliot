...
class Similarity(object):
    ...
    # NUOVA GET USER RECS ----------------------------------------------------------- #
    def get_user_recs(self, u, mask, k):
        # Get internal user ID and user's predictions
        user_id = self._data.public_users.get(u)
        user_recs = self._preds[user_id]

        # Apply mask to filter out unwanted items
        masked_recs = np.where(mask[user_id], user_recs, -np.inf)  # Efficient masking using np.where

        # ------ LOCAL K ------ #
        valid_items = np.sum(mask[user_id])  # Count of valid (non-masked) items
        local_k = min(k, valid_items)  # Adjust k if fewer valid items are available

        # Find the top-k items - using local_k - # Get indices of top-k elements without full sort
        top_k_indices = np.argpartition(masked_recs, -local_k)[-local_k:]
        # ------ END LOCAL K ------ #

        # Find the top-k items - LOCAL K NOT HANDLED
        # top_k_indices = np.argpartition(masked_recs, -k)[-k:]  # Get indices of top k elements without full sort
        # Comment out from "LOCAL K" to "END LOCAL K" and use the above line of code to remove local k

        top_k_values = masked_recs[top_k_indices]  # Fetch values at these indices

        # Sort the top-k items in descending order
        sorted_top_k_indices = top_k_indices[np.argsort(-top_k_values)]  # Descending order sort

        # Map sorted indices back to item IDs and retrieve values
        return [(self._data.private_items[idx], masked_recs[idx]) for idx in sorted_top_k_indices]

    """
    VECCHIA GET_USER_RECS
    def get_user_recs(self, u, mask, k):
        user_id = self._data.public_users.get(u)
        user_recs = self._preds[user_id]
        # user_items = self._ratings[u].keys()
